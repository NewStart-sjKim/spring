...
@RequestMapping("item") //http://localohst:8080/shop1/item/*
public class ItemController {
	@Autowired //(ShopSevice)객체 주입.
	private ShopService service;
	//http://localohst:8080/shop1/item/list
	@RequestMapping("list")
	...
	@RequestMapping({"detail","update","delete"}) //get,post 방식에 상관없이 호출
	...
	public ModelAndView register(@Valid Item item, BindingResult bresult,
		...
		//request : 요청객체 주입.
		//item의 프로퍼티와 파라미터값을 피교하여 같은 이름의 값을 item객체에 저장
		//@Valid : item객체에 입력된 내용을 유효성 검사함 => bresult 검사 결과 저장
		ModelAndView mav = new ModelAndView("item/create"); //view이름 설정
		if(bresult.hasErrors()) { //@Valid 프로세스에 의해서 입력 데이터 오류가 있는경우
	...
	/*
	 * @GetMapping({"detail","update"}) public ModelAndView update(Integer id) {
	 * ModelAndView mav = new ModelAndView(); Item item = service.getItem(id);
